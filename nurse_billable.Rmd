...
# 
# 
# pb_nurse <- pb_nurse_tbl %>% filter(PAT_MRN_ID == "F758917") %>% collect()
# hb_nurse <- hb_nurse_tbl %>% filter(PAT_MRN_ID == "F758917") %>% collect()
# eagle_data <- eagle_data_tbl %>% collect()
# 
# pb_nurse2 <- pb_nurse_tbl %>% filter(PAT_MRN_ID == "A562139") %>% collect()
# hb_nurse2 <- hb_nurse_tbl %>% filter(PAT_MRN_ID == "A562139") %>% collect()
# 
# 
# combined_billing <- combined_billing_tbl %>%
#   filter(PAT_MRN_ID == "F758917") %>%
#   collect()
# 
...
# 
# 
# mutate(service_year = year(ORG_SERVICE_DATE),
#          service_month = month(ORG_SERVICE_DATE)) %>%
#   group_by(service_year, service_month, TRANSACTION_TYPE) %>%
#   summarise(total = sum(AMOUNT, na.rm = TRUE)) %>%
#   collect()
# 
# combined_billing_access <- combined_billing_access_tbl %>%
#   filter(BILLABLE_VS_NONBILLABLE == "Billable") %>%
#   mutate(service_year = year(ORG_SERVICE_DATE),
#          service_month = month(ORG_SERVICE_DATE)) %>%
#   group_by(service_year, service_month, TRANSACTION_TYPE) %>%
#   summarise(total = sum(AMOUNT, na.rm = TRUE)) %>%
